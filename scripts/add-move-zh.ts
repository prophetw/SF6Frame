 ...
 *
 * Usage:
 *   pnpm exec tsx scripts/add-move-zh.ts          # all characters
 *   pnpm exec tsx scripts/add-move-zh.ts ryu      # single character
 */
...
type Move = Record<string, any> & { name: string; nameZh?: string; input?: string; category?: string };

const BUTTON_ZH: Record<string, string> = {
  LP: '轻拳',
  MP: '中拳',
  HP: '重拳',
  LK: '轻脚',
...
const NORMAL_PREFIX_ZH: Record<string, string> = {
  Stand: '站立',
  Crouch: '蹲下',
  Jump: '跳跃',
  'Neutral Jump': '垂直跳跃',
...
const EXACT_NAME_ZH: Record<string, string> = {
  // System
  'Drive Parry': '斗气招架',
...
function translateDirectionalTaunt(name: string): string | undefined {
...
function translateNormalButtonName(name: string): string | undefined {
  // e.g. "Stand LP", "Stand HP (hold)", "Neutral Jump HK"
...
function getMoveNameZh(move: Move): string | undefined {
...
function reorderMove(move: Move): Move {
  const orderedKeys = [
    'name',
    'nameZh',
    'input',
...
function listTargetFiles(characterId?: string): string[] {
...
function main() {
  ...
  for (const filePath of files) {
    ...
    let changed = false;
    for (const move of data.moves as Move[]) {
    ...
    data.moves = (data.moves as Move[]).map(reorderMove);
